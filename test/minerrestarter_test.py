...
from minerrestarter import minerrestarter

class TestStringMethods(unittest.TestCase):

    @patch('minerrestarter.minerrestarter.urllib2.urlopen')
    def testGetHashrate(self, mock_urlopen):
        a = Mock()
        a.read.return_value = hashrateResponse
        mock_urlopen.return_value = a

        hashrate = minerrestarter.get_hashrate("http://example.com", "10s")
        print hashrate
        assert hashrate == 1957.6

    @patch('minerrestarter.minerrestarter.urllib2.urlopen')
    def testGetHashrateEmpty(self, mock_urlopen):
        a = Mock()
        a.read.return_value = hashrateResponse
        mock_urlopen.return_value = a

        hashrate = minerrestarter.get_hashrate("http://example.com", "60s")
        print hashrate
        assert hashrate == 0
